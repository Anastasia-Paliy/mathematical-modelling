...
def Euler_tX(g, x_t, t, dt, g_args):
    """
    Iteration step of the Euler method
    :param g: function name g(t, x, *args)
    = dx/dt
    :param x_t: x(t)
    :param t: t
    :param dt: t_step
...
def Euler_X(f, x_t, dt, f_args):
    """
    Iteration step of the Euler method
    :param f: function name f(x, *args)
    = dx/dt
    :param x_t: x(t)
    :param dt: t_step
    ...
    :return: x(t+dt)
    """
    x_next = x_t + dt * f(x_t, f_args)

    return x_next


def RK4_X(f, x_t, dt, f_args):
    """
    Iteration step of the Runge-Kutta method (4th order)
    :param f: function name f(x, *args) = dx/dt
    :param x_t: x(t)
    :param dt: t_step
    :param f_args: additional positional arguments of the function f (listable object)
    :return: x(t+dt)
    """
    k1 = dt * f(x_t, f_args)
    k2 = dt * f(x_t + k1 / 2, f_args)
    k3 = dt * f(x_t + k2 / 2, f_args)
    k4 = dt * f(x_t + k3, f_args)
    x_next = x_t + (k1 + 2 * k2 + 2 * k3 + k4) / 6
